...
char TinyGPSPlus::handler ()
{
  ...
  case GPS_STATE_START:
    ...
    this->gps_handler_state = GPS_STATE_DATA_ACQ;
  
  case GPS_STATE_DATA_ACQ:
  ...
  case GPS_STATE_DATA_ACQ_EXTENDED:
    this->_processUART (); // Process the SoftUART and do all the necesary things
    if (!(this->_ms_timer_count))
    {
      this->disablePwr ();
      this->gps_handler_state = GPS_STATE_START;
      this->gps_done = true;
      storeDataPointInEEPROM (*this);
    }
  }
}
...
void TinyGPSPlus::mSTimerCallback ()
{
  ...
  if (this->_ms_uart_count) _ms_uart_count --;
}
