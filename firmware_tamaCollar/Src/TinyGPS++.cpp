...
void TinyGPSPlus::_givePulse ()
{
	_delay_ms (300); // Wait for RTC in GPS to stablize
  // Low - High - Low pulse duration is recommended to be 100 mS
...
void TinyGPSPlus::_processUART ()
{
  ...
  if (softuart_kbhit())
    ...
    if ((this->_ms_timer_count < this->_print_ms_tracker) && print_flag)
      {
        this->printData(); print_flag = 0; 
        this->_print_ms_tracker -= 2000;
      }
      //{printf ("GPS Going\r\n"); print_flag = 0;}
    if ((this->_ms_timer_count > this->_print_ms_tracker) && !print_flag)
      print_flag = 1;
...
char TinyGPSPlus::handler ()
{
 ...
 //printf ("Handeling GPS Now\r\n");
  ...
  // Tiny GPS related Init
  this->location.valid = false; // Invalid the location
  this->satellites.val = 0; // Clear the satellite count
  this->satellites.valid = false; // Invalid the satellite count
  
  // Setting timeout to get the GPS Location
  this->_print_ms_tracker = this->_ms_timer_count = GPS_ACQUISITION_WAIT;

  while (this->_ms_timer_count)
  {
    this->_processUART (); // Process the SoftUART and do all the necesary things
    ...
    {
      //if (this->hdop.value() > GPS_HDOP_REQ) // Checks if acquired location qaulity is good enough
      //{
        // If remaining time is greater than GPS_EXTENDED_WAIT, wait for remaining time to get better HDOP
        // But if remaining time is less than GPS_EXTENDED_WAIT, atleat wait for GPS_EXTENDED_WAIT to get better HDOP
        //if (this->_ms_timer_count < GPS_EXTENDED_WAIT) this->_ms_timer_count = GPS_EXTENDED_WAIT;
        this->_ms_timer_count = GPS_EXTENDED_WAIT;
        while (this->_ms_timer_count)
        {
          this->_processUART (); // Process the SoftUART and do all the necesary things
          //if (this->hdop.value() <= GPS_HDOP_REQ) break; // As soon as HDOP is better, leave
        }
      //}
...
{
  if (this->_ms_timer_count) _ms_timer_count --;
}
